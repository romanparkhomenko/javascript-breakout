...
Element = (function() {
  ...
  var get = function(ele) {
    ...
    if (!ele._extended) Object.extend(ele, instance);
    return ele;
  };
...
StateMachine = {
  ...
  create: function(cfg) {
    ...
    if (cfg.initial) {
      // see "initial" qunit tests for examples
      var initial =
        typeof cfg.initial == "string" ? { state: cfg.initial } : cfg.initial; // allow single string to represent initial state, or complex object to configure { state: 'first', event: 'init', defer: true|false }
      name = initial.event || "startup";
      can[name] = ["none"];
      event = this.buildEvent(name, "none", initial.state, target);
      if (initial.defer) target[name] = event;
      // allow caller to trigger initial transition event
      else event.call(target);
    }
  ...
  buildEvent: function(name, from, to, target) {
    return function() {
      if (this.cannot(name))
        throw "event " +
          name +
          " innapropriate in current state " +
          this.current;

      var beforeEvent = this["onbefore" + name];
      if (beforeEvent && false === beforeEvent.apply(this, arguments)) return;

      if (this.current != to) {
        var exitState = this["onleave" + this.current];
        if (exitState) exitState.apply(this, arguments);

        this.current = to;

        var enterState = this["onenter" + to] || this["on" + to];
        if (enterState) enterState.apply(this, arguments);
      }

      var afterEvent = this["onafter" + name] || this["on" + name];
      if (afterEvent) afterEvent.apply(this, arguments);
    };
  }
...
Game = {
  ...
  start: function(id, game, cfg) {
    if (Game.compatible()) {
      return (Game.current = Object.construct(Game.Runner, id, game, cfg).game);
    }
  ...
  loadScript: function(src, cb) {
    ...
    if (Game.ua.isIE) {
      s.onreadystatechange = function(e) {
        if (e.currentTarget.readyState == "loaded") cb(e.currentTarget);
      };
    } else {
      s.onload = function(e) {
        cb(e.currentTarget);
  ...
  Runner: {
    initialize: function(id, game, cfg) {
      ...
      if (this.cfg.state)
        StateMachine.create(
          Object.extend({ target: this.game }, this.cfg.state)
        );
    ...
    initCanvas: function() {
      if (this.game && this.game.initCanvas) this.game.initCanvas(this.front2d);
    },
    ...
    drawStats: function(ctx) {
      if (this.cfg.stats) {
        ctx.fillText(
          this.strings.frame + Math.round(this.stats.count),
          this.width - 100,
          this.height - 60
        );
        ctx.fillText(
          this.strings.fps + Math.round(this.stats.fps),
          this.width - 100,
          this.height - 50
        );
        ctx.fillText(
          this.strings.update + Math.round(this.stats.update) + this.strings.ms,
          this.width - 100,
          this.height - 40
        );
        ctx.fillText(
          this.strings.draw + Math.round(this.stats.draw) + this.strings.ms,
          this.width - 100,
          this.height - 30
        );
      }
    ...
    resize: function() {
      if (
        ...
        this.height != this.front.offsetHeight
      ) {
    ...
    onkey: function(keyCode, mode) {
      var n,
        k,
        i,
        state = this.game.current; // avoid same key event triggering in 2 different states by remembering current state so that even if an earlier keyhandler changes state, the later keyhandler wont kick in.
    ...
    storage: function() {
      try {
        ...
        // IE localStorage throws exceptions when using non-standard port (e.g. during development)
        return (this.localStorage = {});
      }
    },
