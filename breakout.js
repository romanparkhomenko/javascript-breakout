...
Breakout = {
  ...
  initialize: function(runner, cfg) {
    ...
    this.powerups = Object.construct(Breakout.PowerUp, this, cfg.powerups);
    Game.loadSounds({ sounds: cfg.sounds });
  },

  onstartup: function() {
    // the event that fires the initial state transition occurs when Game.Runner constructs our StateMachine
    this.addEvents();
  ...
  initCanvas: function(ctx) {
    // called by Game.Runner whenever the canvas is reset (on init and on resize)
  ...
  ontouchmove: function(ev) {
    if (ev.targetTouches.length == 1) {
      this.paddle.place(
        ev.targetTouches[0].pageX - this.runner.bounds.left - this.paddle.w / 2
      ); // clientX only works in ios, not on android - must use pageX - yuck
    }
  },
  ...
  Score: {
    ...
    load: function() {
      this.highscore = this.game.storage.highscore
        ? parseInt(this.game.storage.highscore)
        : 1000;
    },
    save: function() {
      if (this.score > this.highscore)
        this.game.storage.highscore = this.highscore = this.score;
    },
    ...
    measure: function(ctx) {
      ...
      this.scorefont =
        "bold " + Math.max(9, this.game.court.wall.size - 2) + "pt arial";
      this.highfont =
        "" + Math.max(9, this.game.court.wall.size - 8) + "pt arial";
      ctx.save();
    ...
    draw: function(ctx) {
      if (this.rerender) {
        this.canvas = Game.renderToCanvas(
          this.width,
          this.height,
    ...
    render: function(ctx) {
      ...
      ctx.fillStyle = ishigh
        ? this.game.color.score
        : this.game.color.highscore;
  ...
  Court: {
    ...
    reset: function(level) {
      ...
      var line,
        ...
        y,
        nx,
        ny = Math.min(layout.bricks.length, this.cfg.ychunks);
      this.bricks = [];
      for (y = 0; y < ny; y++) {
    ...
    resize: function() {
      this.chunk = Math.floor(
        Math.min(this.game.width, this.game.height) /
          (Math.max(this.cfg.xchunks, this.cfg.ychunks) + 4)
      ); // room for court plus 2 chunk wall either side
    ...
    remove: function(brick) {
      brick.hit = true;
      if (brick.hasPowerup && brick.hit) {
        switch (Math.floor(Math.random() * 3)) {
          case 0:
            this.game.powerups.extendPaddle();
            break;
          case 1:
            this.game.powerups.expandBall();
            break;
          case 2:
            this.game.powerups.speedUpBall();
            break;
        }
      }
      this.numhits++;
  ...
  Ball: {
    ...
    moveToPaddle: function() {
      this.setpos(
        this.game.paddle.left + this.game.paddle.w / 2,
    ...
    launch: function() {
      if (!this.moving || this.countdown) {
        this.countdown =
          typeof this.countdown == "undefined" || this.countdown == null
            ? 3
            : this.countdown - 1;
        if (this.countdown > 0) {
          this.label = this.launchLabel(this.countdown);
          this.delayTimer = setTimeout(this.launch.bind(this), 1000);
          if (this.countdown == 1) this.setdir(1, -1); // launch on 'go'
        } else {
          this.clearLaunch();
        }
      }
    ...
    launchNow: function() {
      // <space> key can override countdown launch
      if (!this.moving) {
        this.clearLaunch();
        this.setdir(1, -1);
      }
    ...
    update: function(dt) {
      if (!this.moving) return this.moveToPaddle();

      var p2 = Game.Math.move(
        this.x,
        this.y,
        this.dx * this.speed,
        this.dy * this.speed,
      ...
      if (closest) {
        if (closest.item.isbrick) {
          ...
          if (!this.moving)
            // if hitBrick caused game to end we dont want to continue updating our state
            return;
        }
      ...
      if (
        p2.x < 0 ||
        p2.y < 0 ||
        p2.x > this.game.width ||
        p2.y > this.game.height
      ) {
        this.game.loseBall();
      } else {
  ...
  Paddle: {
    ...
    reset: function() {
      ...
      this.setpos(
        Game.random(this.minX, this.maxX),
        this.game.court.bottom - this.h
      );
      this.setdir(0);
      this.rerender = true;
    },
    ...
    update: function(dt) {
      ...
      if (amount != 0) this.place(this.x + amount * dt * this.speed);
    },
  ...
  PowerUp: {
    ...
    extendPaddle: function() {
      const defaultLength = this.game.paddle.w;
      this.game.paddle.w = this.game.paddle.w * 2;
      this.game.paddle.rerender = true;

      setTimeout(() => {
        this.game.paddle.w = defaultLength;
        this.game.paddle.rerender = true;
      }, 5000);
    },

    expandBall: function() {
      const defaultRadius = this.game.ball.radius;
      this.game.ball.radius = this.game.ball.radius * 2;
      this.game.ball.rerender = true;

      setTimeout(() => {
        this.game.ball.radius = defaultRadius;
        this.game.ball.rerender = true;
      }, 5000);
    },

    speedUpBall: function() {
      const defaultSpeed = this.game.ball.speed;
      this.game.ball.speed = this.game.ball.speed * 1.5;
      this.game.ball.rerender = true;

      setTimeout(() => {
        this.game.ball.speed = defaultSpeed;
        this.game.ball.rerender = true;
      }, 5000);
    }
  }
